...
describe('ReferralController', () => {
  ...
  describe('getReferralsByReferrer', () => {
    it('應該返回推薦人的所有推薦記錄', async () => {
      ...
      jest.spyOn(service, 'getReferralsByReferrer').mockResolvedValue(referrals as any);

      // Act
      const result = await controller.getReferralsByReferrer(mockStaffId, 'staff', mockReq);
  ...
  describe('getReferralByPatient', () => {
    it('應該返回患者的推薦記錄', async () => {
      // Arrange
      jest.spyOn(service, 'getReferralByPatient').mockResolvedValue(mockReferral as any);

      // Act
      const result = await controller.getReferralByPatient(mockPatientId, mockReq);

      // Assert
      expect(result).toEqual(mockReferral);
      expect(service.getReferralByPatient).toHaveBeenCalledWith(mockPatientId, mockClinicId);
    });
  ...
  describe('convert', () => {
    it('應該成功轉化推薦', async () => {
      ...
      jest.spyOn(service, 'convertReferral').mockResolvedValue(convertedReferral as any);

      // Act
      const result = await controller.convert(mockReferral.id, convertDto, mockReq);
  ...
  describe('delete', () => {
    it('應該成功取消推薦', async () => {
      ...
      jest.spyOn(service, 'deleteReferral').mockResolvedValue(cancelledReferral as any);
      ...
      expect(service.deleteReferral).toHaveBeenCalledWith(mockReferral.id, mockClinicId);
    });
  });
