...
class BusinessRuleException extends base_exception_1.BaseException {
    ...
    static treatmentRule(rule, message, details) {
        return new BusinessRuleException(`TREATMENT_${rule}`, message, { entity: 'treatment', ...details });
    }
    static revenueRule(rule, message, details) {
        return new BusinessRuleException(`REVENUE_${rule}`, message, { entity: 'revenue', ...details });
    }
    static staffRule(rule, message, details) {
        return new BusinessRuleException(`STAFF_${rule}`, message, { entity: 'staff', ...details });
    }
}
