# 路線圖：Doctor CRM

**版本：** 1.1
**建立日期：** 2026-03-26
**核心目標：** 建立完整的自費醫療療程管理系統，含財務追蹤與患者通知

## 階段概觀

| # | 階段名 | 目標 | 需求 | 成功標準 |
|---|--------|------|------|---------|
| 1 | 13/13 | Complete   | 2026-03-27 | 醫護人員能完整管理療程/醫令，患者能查看進度 |
| 2 | 3/3 | Complete   | 2026-03-27 | 患者在關鍵節點（開始/進度/完成）收到通知 |
| 3 | 3/3 | Complete   | 2026-03-30 | 醫護人員能準確追蹤費用，患者收到清晰發票 |
| 4 | 4/4 | Complete   | 2026-03-31 | 實現與 Doctor Toolbox 雙向同步 |
| 5 | 系統精煉與進階功能 | 提升系統模組化、安全性與自定義能力 | REF-01, SEC-01, DYN-01, SYNC-02 | 1300行Vue拆解完成，全局診所守衛生效，自定義欄位可用 |
| 6 | 核心業務流程重構 | 修復模板、醫令與分潤的斷層 | WORKFLOW-01 | 醫令自動生成、動態分潤生效、回診通知觸發 |
| 7 | 高級數據分析與整合擴展 | 解鎖 JSONB 價值與自動對帳 | ANALYTICS-01, SYNC-03 | 多選/檔案欄位可用、JSONB聚合 API、每日對帳自動修正 |

## Phase 1：療程與醫令核心（約 3-4 週）

**計劃進度：**

| 計劃 | 名稱 | 狀態 | 提交 |
|------|------|------|------|
| 01 | 11/13 | Complete    | 2026-03-27 |
| 02 | TreatmentProgressService + 醫護分配 | Complete    | 2026-03-27 |
| 03 | PatientSearch 實體與搜尋服務 | Complete    | 2026-03-30 |
| 04 | MedicalOrderController 醫令 API | Complete    | 2026-04-01 |
| 05 | TreatmentCourse CRUD API | ✓ 完成 | 9eeb169d |
| 06 | PatientController 患者 API | ✓ 完成 | 98dd91d4 |
| 07 | DTO 驗證層與 ValidationErrorFilter | ✓ 完成 | 00b37172 |
| 08-13 | 後續計劃 | ○ 待處理 | - |

**交付內容：**
- 療程創建/編輯/刪除完整流程
- 醫令管理與使用狀態追蹤
- 患者療程進度可視化
- 患者身份證ID + 姓名索引優化

**技術重點：**
- 優化 TreatmentController 與 TreatmentSessionController
- 改進 ScriptTemplate 的使用狀態管理
- 增強患者搜尋索引

**必達指標：**
- ✓ 療程 CRUD 操作無誤差
- ✓ 療程進度準確反映
- ✓ 患者能快速查詢自己的療程
- ✓ 90% 測試覆蓋率

---

## Phase 2：患者通知系統（約 2-3 週）

**交付內容：**
- 通知系統基礎架構
- 多渠道通知（郵件、簡訊、應用內）
- 療程生命週期事件觸發通知
- 患者通知偏好設定

**技術重點：**
- 使用 NestJS Event Emitter 觸發通知
- 整合郵件服務（nodemailer）
- 整合簡訊服務（待選擇）
- 前端通知 UI 組件

**必達指標：**
- ✓ 關鍵事件都有通知
- ✓ 多渠道發送無故障
- ✓ 患者收到率 > 95%

---

## Phase 3：財務管理完善（約 3-4 週）

**計劃數：** 3 個計劃

**計劃進度：**

| 計劃 | 名稱 | 狀態 | 提交 |
|------|------|------|------|
| 01 | Payment 實體、FeeCalculationService 與 PaymentController（FIN-01/02/05） | ✓ 完成 | c6711537 |
| 02 | Invoice 實體、InvoiceService 與 InvoiceController（FIN-03/04） | ✓ 完成 | 2f52c7f7 |
| 03 | Revenue ReportService 報表 API 與前端 ECharts 視覺化（FIN-06） | ✓ 完成 | cea4d16d |

計劃清單：
- [x] 03-01-PLAN.md — Payment 實體與費用計算服務基礎
- [x] 03-02-PLAN.md — 發票生成與管理系統
- [x] 03-03-PLAN.md — 收入統計報表與前端視覺化

**交付內容：**
- Payment 實體（患者支付記錄，三種付款方式）
- FeeCalculationService（Decimal.js 精確計算，誤差 < 0.01%）
- Invoice 實體（自動流水號，draft/issued/cancelled 狀態流）
- InvoiceService（重複開立防護，費用明細自動生成）
- RevenueReportService（聚合查詢：月趨勢、支付方式、人員分潤）
- 前端報表分頁（ECharts 長條圖 + 環形圖 + 人員表格）

**技術重點：**
- 改進現有 Revenue 模組，新增 Payment + Invoice 實體
- 使用 decimal.js 進行精確財務計算
- TypeORM QueryBuilder 聚合查詢（月趨勢 SQLite strftime）
- ECharts 6.0 + vue-echarts 視覺化

**必達指標：**
- ✓ 費用計算誤差 < 0.01%
- ✓ 發票格式符合台灣稅務要求（invoiceNumber: INV-{YYYYMM}-{序號}）
- ✓ 報表載入時間 < 2 秒

---

## Phase 4：Doctor Toolbox 整合（約 4-5 週）

**計劃數：** 4 個計劃

**計劃進度：**

| 計劃 | 名稱 | 狀態 | Wave | 相依性 |
|------|------|------|------|---------|
| 01 | Webhook 基礎設施 & 患者同步索引 | ✓ 完成 | 1 | None |
| 02 | 雙向同步引擎（衝突解決、重試邏輯）| ✓ 完成 | 1 | 04-01 |
| 03 | 初始診所遷移（批量匯入、進度追蹤）| ✓ 完成 | 2 | 04-01, 04-02 |
| 04 | 審計追蹤 & 監控（日誌、API 文檔） | ✓ 完成 | 2 | 04-01, 04-02, 04-03 |

計劃清單：
- [x] 04-01-PLAN.md — Webhook 基礎設施 & 患者同步索引（INTEGRATION-01, 02）
- [x] 04-02-PLAN.md — 雙向同步引擎（INTEGRATION-02, 03）
- [x] 04-03-PLAN.md — 初始診所遷移（INTEGRATION-01, 04）
- [x] 04-04-PLAN.md — 審計追蹤 & 監控（INTEGRATION-01, 02, 03, 04）

**交付內容：**
- WebhookController（POST /sync/webhook，HMAC-SHA256 簽名驗證）
- SyncPatientIndexEntity（患者身份映射表，精確查詢）
- SyncPatientService（轉換、查詢、衝突解決、合併邏輯）
- RetryService（指數退避重試：2s, 4s, 8s, 16s）
- BulkExportService（批量匯入，進度追蹤，恢復機制）
- SyncAuditLogEntity & SyncAuditService（完整審計追蹤）
- SyncMonitoringService（故障告警、重試分析）
- API 整合文檔（中文 繁體，Webhook 合約、錯誤碼）
- 整合安裝指南（新診所設定步驟）

**技術重點：**
- 使用 NestJS EventEmitter 驅動患者生命週期事件
- HMAC-SHA256 webhook 簽名驗證 + 時戳防重放
- 指數退避重試（無訊息佇列依賴）
- 精確患者匹配（idNumber + name）+ 備用查詢（name + phone）
- CRM 為患者身份權威來源（冪等性設計）
- 多診所隔離（clinicId 篩選）
- 最終一致性同步（異步處理，審計記錄）

**必達指標：**
- ✓ 同步延遲 < 5 秒
- ✓ 資料一致性 99.9%
- ✓ 沒有重複 or 遺漏記錄
- ✓ 測試覆蓋率 ≥ 90%
- ✓ 衝突自動解決（CRM 為主控）

---

## Phase 5：系統精煉與進階功能（約 4-5 週）

**計劃數：** 4 個計劃

**計劃進度：**

| 計劃 | 名稱 | 狀態 | 波次 | 相依性 |
|------|------|------|------|---------|
| 01 | Revenue 視圖重構與 ClinicGuard 安全守衛 | ✓ 完成 | 1 | None |
| 02 | 動態屬性系統（自定義欄位） | ✓ 完成 | 2 | 05-01 |
| 03 | 出站同步引擎（CRM -> Toolbox） | ✓ 完成 | 2 | 05-01 |
| 04 | PostgreSQL 遷移與效能優化 | ✓ 完成 | 3 | 05-01, 05-02, 05-03 |

## Phase 6：核心業務流程重構（Hotfix）

**計劃進度：**

| 計劃 | 名稱 | 狀態 | 相依性 |
|------|------|------|---------|
| 01 | 系統核心業務流程重構 | ✓ 完成 | Phase 5 |

## Phase 7：高級數據分析與整合擴展

**計劃進度：**

| 計劃 | 名稱 | 狀態 | 相依性 |
|------|------|------|---------|
| 01 | 高級數據分析與整合擴展 | ✓ 完成 | Phase 6 |

## 里程碑

| 里程碑 | 目標日期 | 成果 |
|--------|---------|------|
| Phase 1 完成 | 2026-04-23 | 療程/醫令管理生產就緒 |
| Phase 4 完成 | 2026-03-31 | Doctor Toolbox 整合上線 |
| Phase 5 完成 | 2026-04-27 | 系統架構精煉與安全隔離完成 |
| Phase 6 完成 | 2026-04-30 | 核心流程（醫令、分潤、通知）完美連動 |
| Phase 7 完成 | 2026-04-30 | 解鎖 JSONB 價值與自動對帳系統 |
| **v1.3 發佈** | **2026-04-30** | **企業級自費醫療數據決策平台** |

---

*路線圖建立日期：2026-03-26*
*最後更新：2026-04-30（Phase 7 完成）*
